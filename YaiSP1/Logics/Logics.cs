...
namespace YAISP1
{
    class ConHandler
    {
        private List<Doc> _dataBase;
        private Doc _currentDoc;

        public ConHandler()
        {
            _dataBase = new List<Doc>();
        }
        ...
        public void CreateNewDoc()
        {
            ...
            Console.Write(">> ");
            Doc Lst = new Doc(Console.ReadLine());
            _dataBase.Add(Lst);
            _currentDoc = Lst;
            bool GoodToGo = false;
            do
            ...
            ColorText("Активный текст: " + _currentDoc.Header, 'h');
        }
        ...
        private void _AddText(int Type)
        {
            ...
            _currentDoc.AddDiv(Type, Console.ReadLine());
        }
        ...
        public void SetCurrDocMenu()
        {
            ...
            else
            {
                ...
                int Length = _dataBase.Count;   //Получаем количество текстов
                ColorText("Список текстов: ", 'i');
                for (int i = 0; i < Length; i++)
                    ColorText((i + 1) + ". " + _dataBase[i].Header, 'h');
                do
                    Console.Write(">> ");
        ...
        private bool _SetText(int Length)
        {
            ...
            if (int.TryParse(Console.ReadLine(), out TextNum))    //Проверка на численность
            {
                ...
                {
                    _currentDoc = _dataBase[TextNum - 1]; //Т.к. TextNum отображается увеличенным на 1
                    ColorText("Активный текст: " + _currentDoc.Header, 'h');
                    return true;
                }
            }
            ...
            if (_currentDoc != null)
            {
                ...
                if (int.TryParse(Console.ReadLine(), out Width) && Width < 81 && Width > 0)
                {
                    ...
                    if (int.TryParse(Console.ReadLine(), out cols) && cols < 4 && cols > 0)
                        ColorText(_currentDoc.Format(Width, cols), 'o');
                }
                else
